...
export function bridge_commands(l: lightning): command {
	return {
		...
		execute: () =>
			create_message(
				`Try running the help command for help with bridges`
			),
		options: {
			subcommands: [
				{
					...
					execute: async opts => create_message((await join(opts, l))[1]),
					options: { argument_name: 'name', argument_required: true }
				},
				{
					name: 'leave',
					description: 'leave a bridge',
					execute: async opts => create_message((await leave(opts, l))[1])
				},
				{
					name: 'reset',
					description: 'reset a bridge',
					execute: async opts => create_message(await reset(opts, l)),
					options: { argument_name: 'name' }
				},
				{
					name: 'toggle',
					description: 'toggle a setting on a bridge',
					execute: async opts => create_message(await toggle(opts, l)),
					options: { argument_name: 'setting', argument_required: true }
				},
				{
					name: 'status',
					description: 'see what bridges you are in',
					execute: async opts => create_message(await status(opts, l))
				}
			]
		}
	};
